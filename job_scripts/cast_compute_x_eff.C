{
	...
	TString s_readfile = current_path+"_script_running_dir_/"+"_readfiles_name_";
	...
	peak_name = "_peak_name_";
	energy_max = _energy_max_;	energy_min = _energy_min_;
	ch_number = _ch_number_;
	h_energy_binsize = _h_energy_binsize_;
	draw_branch = "_draw_branch_";
	n_multiplicity = _n_multiplicity_;
	n_sigma_for_peak_count = _n_sigma_for_peak_count_;
	n_sigma_for_bg_count = _n_sigma_for_bg_count_;
	peak_index = _peak_index_;
	draw_branch_index = _draw_branchIndex_;
	...
	////////// Set the cuts to draw the histogram
        ...
        TCut x_energy = "Energy > _energy_min_ && Energy< _energy_max_";
	TCut x_tot_energy = "TotalEnergy_RawTime > _tot_energy_threshold_";

	//Good event selection with OT_ChiSquare selection
	TCut x_ot_chisq = "Multiplicity_RawTime > 0";

	//Mutliplicity cut
	TCut x_multiplicity = "Multiplicity_RawTime == _n_multiplicity_";
	...
	cout<<"np: "<<np<<endl;
        ...
        h_energy_zoom_2->SetXTitle("_draw_branch_ [keV]");
	...
	can->cd(1);
        ...
        g_residual->GetXaxis()->SetTitle("_draw_branch_ [keV]");
        g_residual->Draw("ap");
        ...
        g_residual_cut->GetXaxis()->SetTitle("_draw_branch_ [keV]");
	...
	can_fig_name += _n_sigma_for_peak_count_;       can_fig_name +="sigma_peak_range_";
        can_fig_name += list_name;//list_name comes from read_files.C
	//        can_fig_name +="_ch";  can_fig_name += _ch_number_;
        can_fig_name +="_m";  can_fig_name += _n_multiplicity_;
        can_fig_name +=".png";

	can_C_name += peak_name; 	can_C_name += "_count_";
        can_C_name += h_energy_xbin_size;       can_C_name += "keVbin_";
	can_C_name += _n_sigma_for_peak_count_; can_C_name +="sigma_peak_range_";
        can_C_name += list_name;//list_name comes from read_files.C
	//        can_C_name +="_ch";  can_C_name += _ch_number_;
        can_C_name +="_m";  can_C_name += _n_multiplicity_;
        can_C_name +=".C";
	...
	can2->cd(3);
        ...
        can2_fig_name += _n_sigma_for_peak_count_;      can2_fig_name +="sigma_peak_range_";
        can2_fig_name += list_name;
        can2_fig_name +="_m";  can2_fig_name += _n_multiplicity_;
        can2_fig_name +=".png";

        can2_C_name += peak_name;
        can2_C_name +="_fit_";
        can2_C_name += h_energy_xbin_size;      can2_C_name += "keVbin_";
        can2_C_name += _n_sigma_for_peak_count_;        can2_C_name +="sigma_peak_range_";
        can2_C_name += list_name;
        can2_C_name +="_m";  can2_C_name += _n_multiplicity_;
        can2_C_name +=".C";
